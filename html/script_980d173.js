...
let SelectedLowPlanes = false;
let SelectedHighPlanes = false;
let SelectedChangePlanes = false;
let SelectedCirclingPlanes = false;
...
function initPage() {
    ...
    */
    ...
    jQuery('#selectlow_checkbox').on('click', function() {
        if (jQuery('#selectlow_checkbox').hasClass('settingsCheckboxChecked')) {
            deselectLowPlanes();
        } else {
            selectLowPlanes();
        }
...
function refreshPageTitle() {
    ...
    if (PlaneCountInTitle) {
	    ...
	    subtitle += TrackedAircraftPositions
        }
    }
...
(function (global, jQuery, TAR) {
    ...
    cols.altitude = {
        text: 'Altitude',
        sort: function () { sortBy('altitude',compareNumeric, function(x) { return (x.altitude == "ground" ? -100000 : x.getDataSourceNumber() > 1 ? 1000000 : x.altitude); }); },
	...
	$.get('cpuTemp.txt',function(data){
	    cpuTemp = data.substring(5)
	},'text');
	...
	jQuery('#dump1090_cpu_temp').updateText(cpuTemp);	
	...
	//console.log('resortTable:',pList.length,sortExtract,globeIndex)
        if (!sortExtract)
            return;
        if (globeIndex) {
...
function selectAllPlanes() {
    ...
    if (SelectedLowPlanes) { deselectLowPlanes(); selectAllPlanes(); }
    if (SelectedHighPlanes) { deselectHighPlanes(); selectAllPlanes(); }
    if (SelectedChangePlanes) { deselectChangePlanes(); selectAllPlanes(); }
    if (SelectedCirclingPlanes) { deselectCirclingPlanes(); selectAllPlanes(); }
    buttonActive('#T', true);
    // If SelectedPlane has something in it, clear out the selected
    if (SelectedPlane)
        deselect(SelectedPlane);
...
// loop through the planes and mark them as selected to show the paths for low altitude planes
// see planeObject_980d173.js; line 1214.
function selectLowPlanes() {
    //console.log("In selectLowPlanes() function")
    HighlightedPlane = null;
    // if low planes are already selected, deselect them all
    if (SelectedLowPlanes) {
        deselectLowPlanes();
        return;
    }
    if (SelectedAllPlanes) { deselectAllPlanes(); selectLowPlanes(); }
    if (SelectedHighPlanes) { deselectHighPlanes(); selectLowPlanes(); }
    if (SelectedChangePlanes) { deselectChangePlanes(); selectLowPlanes(); }
    if (SelectedCirclingPlanes) { deselectCirclingPlanes(); selectLowPlanes(); }
    buttonActive('#lo', true);
...
// deselect all the planes
function deselectLowPlanes(keepMain) {
    if (showTrace && !keepMain)
        return;
    if (!multiSelect && SelectedPlane)
        toggleIsolation("off");
    ...
    if (SelectedLowPlanes) {
        buttonActive('#lo', false);
        jQuery('#selectall_checkbox').removeClass('settingsCheckboxChecked');
        SelectedLowPlanes = false;
        refreshFilter();
        return;
    }
...
// loop through the planes and mark them as selected to show the paths for high altitude planes
// see planeObject_980d173.js; line 1214.
function selectHighPlanes() {
    //console.log("In selectHighPlanes() function")
    HighlightedPlane = null;
    // if low planes are already selected, deselect them all
    if (SelectedHighPlanes) {
        deselectHighPlanes();
        return;
    }
    if (SelectedAllPlanes) { deselectAllPlanes(); selectHighPlanes(); }
    if (SelectedLowPlanes) { deselectLowPlanes(); selectHighPlanes(); }
    if (SelectedChangePlanes) { deselectChangePlanes(); selectHighPlanes(); }
    if (SelectedCirclingPlanes) { deselectCirclingPlanes(); selectHighPlanes(); }
    buttonActive('#hi', true);
...
// deselect all the planes
function deselectHighPlanes(keepMain) {
    if (showTrace && !keepMain)
        return;
    if (!multiSelect && SelectedPlane)
        toggleIsolation("off");
    ...
    if (SelectedHighPlanes) {
        buttonActive('#hi', false);
        jQuery('#selectall_checkbox').removeClass('settingsCheckboxChecked');
        SelectedHighPlanes = false;
        refreshFilter();
        return;
    }
...
// loop through the planes and mark them as selected to show the paths for planes changing altitude
// see planeObject_980d173.js; line 1214.
function selectChangePlanes() {
    //console.log("In selectChangePlanes() function")
    HighlightedPlane = null;
    // if low planes are already selected, deselect them all
    if (SelectedChangePlanes) {
        deselectChangePlanes();
        return;
    }
    if (SelectedAllPlanes) { deselectAllPlanes(); selectChangePlanes(); }
    if (SelectedLowPlanes) { deselectLowPlanes(); selectChangePlanes(); }
    if (SelectedHighPlanes) { deselectHighPlanes(); selectChangePlanes(); }
    if (SelectedCirclingPlanes) { deselectCirclingPlanes(); selectChangePlanes(); }
    buttonActive('#change', true);
...
// deselect all the planes
function deselectChangePlanes(keepMain) {
    if (showTrace && !keepMain)
        return;
    if (!multiSelect && SelectedPlane)
        toggleIsolation("off");
    ...
    if (SelectedChangePlanes) {
        buttonActive('#change', false);
        jQuery('#selectall_checkbox').removeClass('settingsCheckboxChecked');
        SelectedChangePlanes = false;
        refreshFilter();
        return;
    }
...
// loop through the planes and mark them as selected to show the paths for planes who are circling
// see planeObject_980d173.js; line 1214.
function selectCirclingPlanes() {
    console.log("In selectCirclingPlanes() function")
    HighlightedPlane = null;
    // if low planes are already selected, deselect them all
    if (SelectedCirclingPlanes) {
        deselectCirclingPlanes();
        return;
    }
    if (SelectedAllPlanes) { deselectAllPlanes(); selectCirclingPlanes(); }
    if (SelectedLowPlanes) { deselectLowPlanes(); selectCirclingPlanes(); }
    if (SelectedHighPlanes) { deselectHighPlanes(); selectCirclingPlanes(); }
    if (SelectedChangePlanes) { deselectChangePlanes(); selectCirclingPlanes(); }
    buttonActive('#circle', true);
...
// deselect all the planes
function deselectCirclingPlanes(keepMain) {
    console.log("In deselectCirclingPlanes() function")
    if (showTrace && !keepMain)
        return;
    if (!multiSelect && SelectedPlane)
        toggleIsolation("off");
    ...
    if (SelectedCirclingPlanes) {
        buttonActive('#circle', false);
        jQuery('#selectall_checkbox').removeClass('settingsCheckboxChecked');
        SelectedCirclingPlanes = false;
        refreshFilter();
        return;
    }
