...
PlaneObject.prototype.updateTrack = function(now, last, serverTrack, stale) {
    ...
    // If there is no position, the current and previous position is the identical or bad, don't update the track.
    ...
    // If the lat/lon doesn't make sense, don't update the track.
    ...
    // If there are no segments to the track...
...
PlaneObject.prototype.updateData = function(now, last, data, init) {
    ...
    const gs = isArray? data[2] : data.speed;
...
PlaneObject.prototype.updateFeatures = function(redraw) {
    ...
    if (this.visible) {
		...
		// Find the minimum and maximum altitude in this flight's path.
		let minAlt = 999999;
		let maxAlt = 0;
		for (let i = linesegs.length-1; i >= 1; i--) {
		    let sel = linesegs[i];
		    // Time between segments should be less than 10 minutes.
		    if ((sel.ts - linesegs[i-1].ts) > 600) { break; }
		...
		// Find if the plane's track intersects itself, or is flying in a circle.
		// Look at past segments and compare with everything after that.
		if (linesegs.length > 3) {
		    for (let i = linesegs.length-3; i >= 0; i--) {
			if (circling) { break; }
			else {
		            let sel = linesegs[i];
		            // Time between segments should be less than 10 minutes.
		    ...
		    //if (!circling) { console.log('not circling',this.icao); }
		}    
	    // must be SelectedAllPlanes is true.
...
PlaneObject.prototype.updateLines = function() {
    ...
    this.drawLine = false;
    if (!this.visible || this.position == null || (!this.selected && !SelectedAllPlanes && !SelectedLowPlanes && !SelectedHighPlanes && !SelectedChangePlanes && !SelectedCirclingPlanes)) {
